...
public class GuiAdjustableTransformer extends GuiContainer {
    ...
    public void initGui() {
        ...
        for (int i = 0; i < dirButtons.length; i++) {
            CustomSmallButton button = new CustomSmallButton(i + 16, xLoc + 173, yLoc + yOff + 24 + 13 * i, 32, 13, 27, 192, 27, 207, StatCollector.translateToLocal(Refs.KEY_DIRECTION_NAMES[i]), 4210752, 16777120, guiTexture);
            this.controlList.add(button);
        }
    ...
    protected void drawGuiContainerBackgroundLayer(float var1, int mouseX, int mouseY) {
        ...
        // Draw stats text
        RenderUtils.drawRightAlignedText(fontRenderer, StatCollector.translateToLocal("inv.electric.avg.eu"), xLoc + 180, yLoc + 26, 4210752);
        RenderUtils.drawRightAlignedText(fontRenderer, StatCollector.translateToLocal("inv.electric.avg.packet.in"), xLoc + 180, yLoc + 36, 4210752);
        RenderUtils.drawLeftAlignedText(fontRenderer, StatCollector.translateToLocal("inv.electric.eu.bugger"), xLoc + 49, yLoc + 26, 4210752);
        ...
        RenderUtils.drawCenteredText(fontRenderer, StatCollector.translateToLocal("inv.electric.packet.size"), xLoc + 88, yLoc + yOff + 21, 0xB00000);
        ...
        fontRenderer.drawString(StatCollector.translateToLocal("inv.eu"), xLoc + 152, yLoc + yOff + 49, 4210752);

        // Transfer rate section text
        RenderUtils.drawCenteredText(fontRenderer, StatCollector.translateToLocal("inv.electric.transfer.max"), xLoc + 88, yLoc + yOff + 64, 0xB00000);
        ...
        fontRenderer.drawString(StatCollector.translateToLocal("inv.eu"), xLoc + 152, yLoc + yOff + 92, 4210752);

        // Side input/output settings text
        for (int i = 0; i < 6; i++) {
            RenderUtils.drawGlowingText(fontRenderer, StatCollector.translateToLocal((tile.sideSettings[i] & 1) == 0 ? "inv.in" : "inv.out"), xLoc + 214, yLoc + yOff + 27 + 13 * i, GREEN, GREEN_GLOW);
        }
    }
